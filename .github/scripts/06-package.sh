...
if [[ ${GITHUB_BASE_REF} =~ "release" ]]; then
    DISTNAME="hive-${VERSION}"
else
    DISTNAME="hive-${VERSION}-${SHORTHASH}"
fi
...
if [[ ${OS} == "windows" ]]; then
    ...
    for rmfile in detach-sig-create.sh win-codesign.cert hive-cli.exe hive-qt.exe hived.exe; do
        if [[ -e ${rmfile} ]]; then
            rm -f ${rmfile}
        fi
    done
    ...
    if [[ -e ${GITHUB_WORKSPACE}/dist/Hive-Qt.app/Contents/MacOS/install_cli.sh ]]; then
        chmod +x ${GITHUB_WORKSPACE}/dist/Hive-Qt.app/Contents/MacOS/install_cli.sh
    fi
    ...
    ${GITHUB_WORKSPACE}/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Hive-Core.dmg" ${RELEASE_LOCATION}/${DISTNAME}-osx-unsigned.dmg
    ...
    if [[ -e ${RELEASE_LOCATION} ]]; then
        ...
        cd ${STAGE_DIR}
        cp -Rf ${DISTNAME}/bin/hived .
        cp -Rf ${DISTNAME}/bin/hive-cli .
    else
        echo "release directory doesn't exist"
    fi
else
