...
const std::string HiveGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString HiveGUI::DEFAULT_WALLET = "~Default";

HiveGUI::HiveGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Hive command-line options").arg(tr(PACKAGE_NAME)));
...
void HiveGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/hivecointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* hiveLabelLayout = new QVBoxLayout(toolbarWidget);
        hiveLabelLayout->addWidget(label);
        hiveLabelLayout->addWidget(toolbar);
        hiveLabelLayout->setDirection(QBoxLayout::TopToBottom);
        hiveLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Hivecoin Market Price"));
        ...
        priceLayout->addWidget(labelBtcHvn, 0 , Qt::AlignVCenter | Qt::AlignLeft);
        ...
        // Get the latest Hivecoin release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void HiveGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool HiveGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool HiveGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void HiveGUI::setWalletActionsEnabled(bool enabled)
{
...
void HiveGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void HiveGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void HiveGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void HiveGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void HiveGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void HiveGUI::openClicked()
{
...
void HiveGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void HiveGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void HiveGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void HiveGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void HiveGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void HiveGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** HVN START */
void HiveGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void HiveGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void HiveGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void HiveGUI::gotoRestrictedAssetsPage()
{
...
void HiveGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Hive network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void HiveGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void HiveGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void HiveGUI::updateHeadersSyncProgressLabel()
{
...
void HiveGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void HiveGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Hive"); // default title
...
void HiveGUI::changeEvent(QEvent *e)
{
...
void HiveGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void HiveGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void HiveGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
...
void HiveGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void HiveGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool HiveGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool HiveGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void HiveGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void HiveGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void HiveGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void HiveGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void HiveGUI::detectShutdown()
{
...
void HiveGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void HiveGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void HiveGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(HiveGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(HiveGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void HiveGUI::subscribeToCoreSignals()
{
...
void HiveGUI::unsubscribeFromCoreSignals()
{
...
void HiveGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void HiveGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<HiveUnits::Unit> units = HiveUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const HiveUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(HiveUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(HiveUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (HiveUnits::Unit u : HiveUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(HiveUnits::name(u)), this);
...
{
    setText(HiveUnits::name(newUnits));
}
