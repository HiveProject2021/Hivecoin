    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/HiveProject2021/Hivecoin/issues/5716
...
{
    GUIUtil::setClipboard(HiveUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void AssetControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = HiveUnits::HVN;
    ...
    l2->setText(HiveUnits::formatWithCustomName(QString::fromStdString(strAssetName), nAssetAmount));        // Amount
    l3->setText(HiveUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(HiveUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(HiveUnits::formatWithCustomName(QString::fromStdString(strAssetName), nChange));        // Change
    if (nPayFee > 0)
    {
...
void AssetControlDialog::updateView()
{
    ...
    for (const std::pair<QString, std::vector<COutput>> &coins : mapAssetCoins) {
        ...
        for (const COutput &out : coins.second) {
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, HiveUnits::format(nDisplayUnit, nAmount));
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, HiveUnits::format(nDisplayUnit, nSum));
