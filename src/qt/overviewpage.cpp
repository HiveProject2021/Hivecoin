...
public:
    explicit TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(HiveUnits::HVN),
    ...
    assetdelegate(new AssetViewDelegate(platformStyle, this))
{
    ...
    ui->hvnBalancesLabel->setStyleSheet(STRING_LABEL_COLOR);
    ...
    ui->hvnBalancesLabel->setFont(GUIUtil::getTopLabelFont());
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(HiveUnits::formatWithUnit(unit, balance, false, HiveUnits::separatorAlways));
    ui->labelUnconfirmed->setText(HiveUnits::formatWithUnit(unit, unconfirmedBalance, false, HiveUnits::separatorAlways));
    ui->labelImmature->setText(HiveUnits::formatWithUnit(unit, immatureBalance, false, HiveUnits::separatorAlways));
    ui->labelTotal->setText(HiveUnits::formatWithUnit(unit, balance + unconfirmedBalance + immatureBalance, false, HiveUnits::separatorAlways));
    ui->labelWatchAvailable->setText(HiveUnits::formatWithUnit(unit, watchOnlyBalance, false, HiveUnits::separatorAlways));
    ui->labelWatchPending->setText(HiveUnits::formatWithUnit(unit, watchUnconfBalance, false, HiveUnits::separatorAlways));
    ui->labelWatchImmature->setText(HiveUnits::formatWithUnit(unit, watchImmatureBalance, false, HiveUnits::separatorAlways));
    ui->labelWatchTotal->setText(HiveUnits::formatWithUnit(unit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance, false, HiveUnits::separatorAlways));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("HVN")
    updateDisplayUnit();
}
...
void OverviewPage::showAssets()
{
    if (AreAssetsDeployed()) {
        ...
        // This keeps the HVN balance grid from expanding and looking terrible when asset balance is hidden
