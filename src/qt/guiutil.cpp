...
#include "hiveaddressvalidator.h"
#include "hiveunits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Hive address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new HiveAddressEntryValidator(parent));
    widget->setCheckValidator(new HiveAddressCheckValidator(parent));
}
...
bool parseHiveURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no hive: URI
    if(!uri.isValid() || uri.scheme() != QString("hive"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!HiveUnits::parse(HiveUnits::HVN, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseHiveURI(QString uri, SendCoinsRecipient *out)
{
    // Convert hive:// to hive:
    //
    //    Cannot handle this later, because hive:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("hive://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "hive:");
    }
    QUrl uriInstance(uri);
    return parseHiveURI(uriInstance, out);
}

QString formatHiveURI(const SendCoinsRecipient &info)
{
    QString ret = QString("hive:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(HiveUnits::format(HiveUnits::HVN, info.amount, false, HiveUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Hive (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Hive (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("hive-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            ...
            optionFile << strprintf("Name=Hive (%s)\n", chain);
...
bool GetStartOnSystemStartup()
{
    CFURLRef hiveAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (hiveAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, hiveAppUrl);

    CFRelease(hiveAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        ...
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, hiveAppUrl, nullptr, nullptr);
    }
