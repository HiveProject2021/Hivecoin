...
UniValue transfer(const JSONRPCRequest& request)
{
    ...
    if (!hvn_change_address.empty() && !IsValidDestination(hvn_change_dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("HVQ change address must be a valid address. Invalid address: ") + hvn_change_address);
...
UniValue listtagsforaddress(const JSONRPCRequest &request)
{
    ...
    CTxDestination dest = DecodeDestination(address);
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Not valid HVQ address: ") + address);
