...
sudo find ${DISTNAME}/bin -type f -executable -exec ../hivecoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../hivecoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/hive-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer hive-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer hive-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
#transfer hive-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/hivecoin/contrib/macdeploy/extract-osx-sdk.sh
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/hive-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer hive-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
