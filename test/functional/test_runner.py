...
`test/functional/test_framework/test_framework.py:HiveTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/hive_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and hived
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_hived = config["components"].getboolean("ENABLE_HIVED")
    if not (enable_wallet and enable_cli and enable_hived):
        print("No functional tests to run. Wallet, utils, and hived must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if hived is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "hived"]) is not None:
            print("%sWARNING!%s There is already a hived process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "HIVED" not in os.environ:
        os.environ["HIVED"] = build_dir + '/src/hived' + exeext
        os.environ["HIVECLI"] = build_dir + '/src/hive-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `hive-cli help` (`rpc_interface.txt`).
