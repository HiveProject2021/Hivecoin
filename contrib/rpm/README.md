...
The RPM spec file provided here is for Hive-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://hivecoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Hive git repository](https://github.com/HiveProject2021/Hivecoin) but are not part of
...
Currently only `Source10` is used, for the example `hive.conf` file.
...
in the hive git repository. At present only an SVG version of the Hive
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the hive
...
needed when 0.12.1 is released, a proper fix is already in the Hive git
master branch.
...
idea to build Hive Core against a static version of that library compiled
...
The `hive.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `hive-server` package. At this point, most RPM based
...
The `hive-server` package should have SELinux support. How to properly do
...
Hive-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/hive/). He has been
packaging Hive for Fedora at least since 2012.
...
4. I renamed the `hive` package that contains the Qt GUI to `hive-core` as
that appears to be how the general population refers to it, in contrast to
`hive-xt` or `hive-classic`. I wanted to make sure the general population
