...
The Hivecoin Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play hives" on the test network, if you
...
- HiveMiner : Generates hives (if wallet is enabled).
...
Some of these are maintained by active developers of Hive Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Hive Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/hive-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/hive-core/ctaes ; actively maintained by Core contributors.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `hive-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `hive-cli` and the GUI debug console use this table to determine how to
